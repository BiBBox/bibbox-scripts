...
usage()
{
    ...
    echo ""
    echo "OPTIONS:"
    echo "      -t, --toolname                  Unique name of the tool"
    echo ""
    echo "Example:"
...
createProxyfile()
{
    if [ -e /etc/apache2/sites-available ]; then
        echo "Create proxy file /etc/apache2/sites-available/005-$toolname.conf"
    else
        error_exit "$LINENO: Directory sites-available from apache2 not available! Aborting."
    fi
    
cat > /etc/apache2/sites-available/005-$toolname.conf <<EOF
<VirtualHost *:80>
...
linkfile()
{
  ln -s /etc/apache2/sites-available/005-$toolname.conf /etc/apache2/sites-enabled/005-$toolname.conf
}
